...
def brightn_profile_subtract(data, center, rad_range):
    y, x = np.indices((data.shape))
    r = np.sqrt((x - center[0])**2 + (y - center[1])**2)
    cond = (r > rad_range[0]) & (r < rad_range[1])
    inds = zip(*np.where(cond))
    ring=[]
    for i in range(len(inds)):
        ring.append(data[inds[i]])
    for i in range(len(inds)):
        data[inds[i]] -= np.median(ring)
    print '=================='
    return data
...
for i in range(len(filepath)):
    ...
    for j in range(len(radz)-1):
#        imaj = brightn_profile_subtract(imaj, [x0[i],y0[i]], [radz[j],radz[j+1]])
        imaj = brightn_profile_subtract(imaj, [512,512], [radz[j],radz[j+1]])
...
fname = ['bpstk_roxs12','bpstk_roxs42','bpmed_roxs12','bpmed_roxs42']
